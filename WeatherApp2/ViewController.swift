...
//  Weather App
//
//  Created by KayqueDosAnjos on 02/08/24.
//
...
class ViewController: UIViewController {
    ...
    private var city = City(lat: "-23.5505", lon: "-46.6333", Name: "São Paulo")
...
extension ViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.loadData(time: forecast?.dt.toHourFormat() ?? "0",
                      icon: UIImage(named: forecast?.weather.first?.icon ?? "")!,
                      temp: forecast?.temp.toCelsius() ?? "0")
        return cell
    }
}
